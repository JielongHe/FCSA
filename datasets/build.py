...
from datasets.sampler import RandomIdentitySampler,RandomSampler
...
def build_transforms(args, img_size=(384, 128), aug=False, is_train=True):
    ...
    if aug:
        ...
        transform_with_mask = T.Compose([
            ...
            T.ToTensor(),
            T.Normalize(mean=mean, std=std),
            T.RandomErasing(scale=(args.i_mask_min, args.i_mask_max), value=mean),
        ])
        return transform, transform_with_mask
    else:
        transform = T.Compose([
            T.Resize((height, width)),
            T.RandomHorizontalFlip(0.5),
...
def build_dataloader(args, tranforms=None):
    ...
    if args.training:
        train_transforms, transform_with_mask = build_transforms(args,img_size=args.img_size,
                                            aug=args.img_aug,
                                            is_train=True)
        val_transforms = build_transforms(args,img_size=args.img_size,
                                          is_train=False)

        pid_dict = defaultdict(list)
        for idx, item in enumerate(dataset.train):
            pid_dict[item[0]].append(idx)
        ...
        if tranforms:
            ...
            test_transforms = build_transforms(args,img_size=args.img_size,
                                               is_train=False)
